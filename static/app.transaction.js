App.populator('transaction', function(page, args) {
	var transaction = args.id && CashTrack.request('transaction', args.id);
	
	function save() {
		var errors = {};
		transaction = transaction || CashTrack.request('transaction.create');
		transaction.amount = $(page).find('.amount').val();
		transaction.destination= $(page).find('.category.selected').data('category');
		transaction.date = $( page ).find('.date').data('date');
	...
	if( transaction ) {
		$(page).find('.delete').click(function() {
			CashTrack.request('transaction.remove', transaction);
			App.back();
		});
	} else {
		$(page).find('.delete').hide();
	}
	
	$( page ).find('.amount')
		.val(Globalize.format( transaction ? transaction.amount : 0.0, 'n2'))
	...
	_.each( CashTrack.request('categories'), function(category) {
		var item = $(template[0].cloneNode(true));
			...
			if( (!transaction && category.default) || ( transaction && transaction.destination == category.id ) ) {
	...
	if( transaction ) {
		var x = $( page ).find('.date').pickadate('picker').set( 'select', transaction.date );
		//.set().val('Heute');
	} else
